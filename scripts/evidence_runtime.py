#!/usr/bin/env python3
"""Optional evidence-provider runtime with explicit activation boundaries."""
...
def provider_manifests(include_nonoperational: bool = False):
    """Return provider manifests; normal discovery exposes runtime-eligible defaults only."""
    out = []
    for path in sorted(EVIDENCE_ROOT.glob("*/provider.json")):
        if path.parent.name.startswith("_"):
            continue
        item = load_json(path)
        item["_manifest_path"] = str(path.relative_to(ROOT))
        if include_nonoperational or (
            item.get("status") == "OPERATIONAL" and item.get("enabled_by_default", False)
        ):
            out.append(item)
    return out
...
def normalize_evidence(item, manifest):
    ...
    out.setdefault("strength", "UNSPECIFIED")
...
def run_provider(manifest, text, context=None, *, hard_timeout_ms=None):
    ...
    proc = ctx.Process(
        target=_worker,
        args=(module_path, text, context or {}, timeout_ms, q),
        daemon=True,
    )
    proc.start()
    ...
    if not payload.get("ok"):
        status = _status(
            ...
            "UNAVAILABLE",
            started,
            message=f"{payload.get('error_type', 'Error')}: {payload.get('message', '')}",
        )
        if manifest.get("failure_policy", "SKIP") == "ERROR":
            raise RuntimeError(f"{pid}: {status['message']}")
        return [], status
...
def _resolve_requested(provider_ids):
    ...
    all_items = provider_manifests(True)
    by_id = {m["id"]: m for m in all_items}

    if provider_ids == "auto":
        return [
            m
            for m in all_items
            if m.get("status") == "OPERATIONAL" and m.get("enabled_by_default", False)
        ]
    if provider_ids == "all":
        return [m for m in all_items if m.get("status") == "OPERATIONAL"]
    ...
    unavailable = sorted(
        m["id"]
        for m in requested
        if m.get("status") not in {"PROJECT", "OPERATIONAL"}
    )
    if project_only:
        raise ValueError(
...
def run_evidence(text, provider_ids, *, context=None, budget_ms=None):
    ...
    budget_ms = max(
        25,
        int(budget_ms or os.getenv("HUMANIZER_EVIDENCE_BUDGET_MS", DEFAULT_BUDGET_MS)),
    )
    ...
    for manifest in manifests:
        ...
        if remaining <= 0:
            ...
            continue
        items, status = run_provider(
            manifest,
            text,
            context=context,
            hard_timeout_ms=min(int(manifest.get("timeout_ms", remaining)), remaining),
        )
...
def _self_test():
    ...
    items, status = run_provider(project, "текст")
    assert items == [] and status["status"] == "UNAVAILABLE"
    ...
    fast = {
        ...
        "failure_policy": "SKIP",
        "timeout_ms": 250,
        "module_path": "tests/fixtures/evidence_fast.py",
    }
    items, status = run_provider(fast, "Ну вот.", context={"findings": []})
    assert status["status"] == "OK" and items and items[0]["provider_id"] == "fast", status
