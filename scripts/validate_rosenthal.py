...
def main():
    ...
    p4=load('libraries/rosenthal/provenance-cycle4.json')
    ...
    o4=load('studies/rosenthal-pravopisanie-stilistika/observations.json')
    ...
    if idx['source_cycles']!=4 or idx['total_rule_count']!=74 or len(items)!=74: raise SystemExit('expected 4 cycles / 74 cumulative Rosenthal rules')
    ...
    if p4['existing_rules_enriched']!=50 or len(p4['map'])!=50 or p4.get('new_rule_ids')!=[]: raise SystemExit('cycle4 provenance-only contract mismatch')
    existing=set(ids)
    for row in p3['map']:
    ...
    if len(manifest.get('rule_groups',[]))!=3 or len(manifest.get('provenance_maps',[]))!=3: raise SystemExit('cycle4 cumulative routing missing')
    ...
    if idx['groups']!=manifest['rule_groups']: raise SystemExit('index/manifest rule groups diverged')
    ...
    for study_name in ['rosenthal-a-kak-luchshe-skazat','rosenthal-pravopisanie-proiznoshenie-redaktirovanie','rosenthal-pravopisanie-stilistika']:
        study=ROOT/'studies'/study_name
        for name in required:
            if not (study/name).is_file(): raise SystemExit(f'missing study artifact: {study_name}/{name}')
    for study_name,payload in [('rosenthal-pravopisanie-proiznoshenie-redaktirovanie',o3),('rosenthal-pravopisanie-stilistika',o4)]:
    ...
    e4=load('studies/rosenthal-pravopisanie-stilistika/evals.json')['cases']; m4=load('studies/rosenthal-pravopisanie-stilistika/eval-map.json')['map']
    if len(e4)!=12 or {x['id'] for x in e4}!={x['eval_id'] for x in m4}: raise SystemExit('cycle4 eval map mismatch')
    if ({x['rule_id'] for x in m3}|{x['rule_id'] for x in m4})-{x['rule_id'] for x in items}: raise SystemExit('eval maps unknown Rosenthal rules')
    ...
    print(f'rosenthal cumulative library: {len(items)} rules across 4 sources; automation {counts}; cycle4 58 observations, 0 new rules, 50 enriched rules, {len(e4)} provenance/preservation evals; OK')
